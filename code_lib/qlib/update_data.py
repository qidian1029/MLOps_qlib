...
def csv_bin(dump_path,csv_path,qlib_dir):
    ...
    return print("bin格式数据已存储到:",qlib_dir)
...
def ak_data_to_csv(config):
    ...
    for stock_code, start_date, end_date in tqdm(stock_info,desc='下载股票数据'):
        ...
        adj_factor = adj_factor.loc[~adj_factor.index.duplicated(keep='first')]
